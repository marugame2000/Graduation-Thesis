...
random_h_values_low = np.arange(12, 100, 1)
random_h_values_middle = np.arange(100, 1000, 10)
random_h_values_high = np.arange(1000, 10000, 100)
...
filtered_differences_low = [diff for h, diff in differences if 12 < h < 100]
average_difference_low = sum(filtered_differences_low) / len(filtered_differences_low) if filtered_differences_low else 0
filtered_differences_middle = [diff for h, diff in differences if 100 < h < 1000]
average_difference_middle = sum(filtered_differences_middle) / len(filtered_differences_middle) if filtered_differences_middle else 0
filtered_differences_high = [diff for h, diff in differences if 1000 < h < 10000]
average_difference_high = sum(filtered_differences_high) / len(filtered_differences_high) if filtered_differences_high else 0
filtered_differences_very_high = [diff for h, diff in differences if 10000 < h < 100000]
average_difference_very_high = sum(filtered_differences_very_high) / len(filtered_differences_very_high) if filtered_differences_very_high else 0
...
plt.xlabel('h value', fontsize=18)
...
log_data = {
    ...
    'average_difference_low': average_difference_low,
    'average_difference_middle': average_difference_middle,
    'average_difference_high': average_difference_high,
    'average_difference_very_high': average_difference_very_high
}
