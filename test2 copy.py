...
orthogonality_penalty_weight = 0.2
edge_penalty_weight = 1e15
normalization_penalty_weight=1
...
class TrainingHistory(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        self.normalization_penalty.append(K.get_value(normalization_penalty_global))
...
normalization_penalty_global= K.variable(0.0)
...
def variationalE(y_true, y_pred):
    ...
    orthogonality_penalty2 = K.sum(second_excited_answer_minus * wave_nom) **2 
    ...
    #wave_nom = K.l2_normalize(wave, axis=0)
    wavefunction_difference_penalty = K.sum(K.square(wave_nom))
    ...
    normalization_penalty = K.max(wave_nom)
    normalization_penalty_global.assign(normalization_penalty)

    return kinetic_energy + orthogonality_penalty + edge_penalty + normalization_penalty*0 + (orthogonality_penalty2 - 1 ) *0 + wavefunction_difference_penalty *0

a=0.01

model = Sequential([
    Dense(256, input_dim=1, activation=LeakyReLU(alpha=a)),
    Dense(256, activation=LeakyReLU(alpha=a)),
    Dense(256, activation=LeakyReLU(alpha=a)),
    Dense(128, activation=LeakyReLU(alpha=a)),
    Dense(128, activation=LeakyReLU(alpha=a)),
    Dense(128, activation=LeakyReLU(alpha=a)),
    Dense(64, activation=LeakyReLU(alpha=a)),
    Dense(64, activation=LeakyReLU(alpha=a)),
    Dense(64, activation=LeakyReLU(alpha=a)),
    Dense(64, activation=LeakyReLU(alpha=a)),
...
plt.xlabel('Epochs')
plt.ylabel('Orthogonality Penalty2')
plt.ylim(0, 200)
plt.title('Orthogonality Penalty During Training')
plt.show()
...
plt.plot(training_history.normalization_penalty)
plt.xlabel('Epochs')
plt.ylabel('normalization_penalty')
plt.ylim(0, 10)
plt.title('Kinetic Energy During Training')
plt.show()
